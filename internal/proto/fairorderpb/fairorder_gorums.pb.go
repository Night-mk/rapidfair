...
// QuorumSpec is the interface of quorum functions for FairCollect.
type QuorumSpec interface {
	gorums.ConfigOption
}

// FairCollect is the server-side API for the FairCollect Service
type FairCollect interface {
	Collect(ctx gorums.ServerCtx, request *CollectTxSeq)
}

func RegisterFairCollectServer(srv *gorums.Server, impl FairCollect) {
	srv.RegisterHandler("fairorderpb.FairCollect.Collect", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
...
func (n *Node) Collect(ctx context.Context, in *CollectTxSeq, opts ...gorums.CallOption) {
	cd := gorums.CallData{
		Message: in,
		Method:  "fairorderpb.FairCollect.Collect",
	}
